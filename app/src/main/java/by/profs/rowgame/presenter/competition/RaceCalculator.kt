...
class RaceCalculator(private val raceType: Int) {
    ...
    fun calculateRace(
        ...
        val chances: MutableList<Int> = MutableList(rowers.size) { pos ->
            if (raceType.isWaterCompetition()) calculatePower(boats[pos], oars[pos], rowers[pos])
    ...
    private fun calculatePower(boat: Boat? = null, oar: Oar? = null, rower: Rower): Int {
        ...
        when (raceType) {
            ...
            CompetitionType.CONCEPT.ordinal -> power = rower.power +
                    (rower.technics / KEY_SKILL_COEF + rower.endurance * KEY_SKILL_COEF).toInt()
            CompetitionType.OFP.ordinal -> power = rower.endurance +
    ...
    companion object {
        ...
        private val maxGap: HashMap<Int, Int> = hashMapOf(
            CompetitionType.CONCEPT.ordinal to 40,
            CompetitionType.OFP.ordinal to 57,
            CompetitionType.WATER.ordinal to 15
        )
    }
}